...
def main():
    ...
    multiplos_14 = [[] for _ in range(115)]  # Lista para armazenar os múltiplos de 14
    pagou = [[] for _ in range(115)]  # Lista para armazenar as mensagens de pagamento
    multiplos_14_dict = {}  # Dicionário para mapear os horários dos múltiplos de 14 aos seus índices
    multiplos_14_before = [[] for _ in
                           range(115)]  # Lista para armazenar os horários um minuto antes dos múltiplos de 14
    multiplos_14_after = [[] for _ in
                          range(115)]  # Lista para armazenar os horários um minuto depois dos múltiplos de 14
    ...
    for i in range(14, 115, 14):  # Altere 197 para 226
    ...
    df["Pagador"] = df["Pagador"].apply(lambda
                                            x: f'<div style="position: relative; text-align: center;"><img src="./static/ciclo1.png" width="60" height="60"><span style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: white;">{x[:-1]}</span></div>' if isinstance(
    ...
    columns = [str(i) for i in range(14, 115, 14)]  # Cria uma lista com os nomes das colunas
    ...
    html = html.replace('<td',
                        '<td style="width: 80px;text-align: center; background-color: #0f1923; color: white; font-size: 0.8em; font-weight: bold; border: 2px solid #1a242d; padding: 0;"')
