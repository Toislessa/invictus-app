...
print('iniciando intervalosFinal')
...
def blaze_now_results():
    ...
    if resultado['status'] == 'rolling':
        ...
        if before_result != current_result:
            ...
            last_sequence.append(current_result)
            # Aqui você pode chamar a função para atualizar o DataFrame com a nova sequência
            update_dataframe()
...
schedule.every(1).seconds.do(run_threaded, blaze_now_results)

def main():
    print("Running intervalosFinal Main(1)")
    sequence = get_sequence()  # Obtenha a sequência em tempo real
    print("Running intervalosFinal Main(2)")
    ...
    return df
    # Inicie a função blaze_now_results em um novo thread
    threading.Thread(target=blaze_now_results).start()


# Chame a função main quando o script for iniciado
main()

def update_dataframe():
    # Aqui você deve adicionar o código para atualizar o DataFrame com a nova sequência
    # Como um exemplo, estou apenas imprimindo a nova sequência
    print('Updating DataFrame with new sequence:', current_result)
