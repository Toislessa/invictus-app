...
# Inicio Colocar "P" nos multiplos de 14
    ...
    for i, row in df.iterrows():
        ...
        if row['Branco'] is not None and not any("X" in multiplo for multiplo in row['multiplos']):
            ...
            for j, multiplos in df['multiplos'].items():
                ...
                if zero_time_2 in multiplos and not any("X" in multiplo for multiplo in multiplos):
        ...
        if row['Branco'] is not None and not any("P" in multiplo for multiplo in row['multiplos']) and not any("X" in multiplo for multiplo in row['multiplos']):
            print(f"  multiplos: {row['multiplos']}")
            previous_time = row['Branco']
            for j, multiplos in df['multiplos'].items():
                print(f"  Processando célula {j}...")
                if j > i:
                    continue
                for shift, label in [(-1, 'Depois'), (1, 'Antes')]:  # Busca pelos horários com minuto antes e depois
                    zero_time_2 = get_time_shifted(row['Branco'], shift)
                    print(f"  Verificando zero_time: {zero_time_2}")
                    if zero_time_2 in multiplos and not any("P" in multiplo for multiplo in multiplos) and not any("X" in multiplo for multiplo in row['multiplos']):
                        print(f"  Encontrado {zero_time_2} em multiplos!")
