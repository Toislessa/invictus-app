...
async def main():
    ...
    cycle_numbers = set(range(1, 28))  # Conjunto de números para um ciclo completo
    ...
    semaphore = asyncio.Semaphore(13)  # limita o número de solicitações simultâneas a 5
    ...
    for page in pages:
        ...
        for resultado in resultados:
            ...
            if (created_at_datetime_2.time() >= dt_time(0, 0) and created_at_datetime_2.date() == (
            ...
            if roll == 0:
                if ((created_at_datetime_2.time() >= dt_time(0, 0) and created_at_datetime_2.date() == (
                    ...
                    # Adicione os múltiplos de 28 a esta lista
                    for i in range(28, 520, 28):
                        if created_at_str_2 is not None:  # Verifique se created_at_str_2 não é None
                            next_time = (datetime.strptime(created_at_str_2, "%H:%M") + timedelta(minutes=i)).strftime(
                                "%H:%M")
                            multiplos_28.append(next_time)

                    print(f"Lista de múltiplos criada para este zero: {multiplos_28}")

                    # Adicione esta lista de múltiplos à lista de todas as listas de múltiplos
                    todas_as_listas_de_multiplos.append(multiplos_28)
    ...
    # Adicione as colunas "224" a "28" ao DataFrame df2
    for i in range(28, 520, 28):  # Altere 197 para 226
        # Crie uma lista para armazenar os valores da coluna atual
        col_values = [get_ith_multiple(multiplos, i // 28 - 1) for multiplos in todas_as_listas_de_multiplos]
    ...
    multiples_of_28_cols = list(range(28, 520, 28))
    ...
    for col in multiples_of_28_cols:
    ...
    # Inicio Colocar "P" nos multiplos de 28
    index_to_column = {i: str(28 * (i + 1)) for i in range(504 // 28)}
    previous_time = None
    ...
    columns = [str(i) for i in range(28, 520, 28)]  # Cria uma lista com os nomes das colunas
    ...
    html_df3 = html_df3.replace('>E28<', '><img src="./static/E28.png" width="100%" height="100%" /><')
    html_df3 = html_df3.replace('>E28<', '><img src="./static/E28.png" width="100%" height="100%" /><')
